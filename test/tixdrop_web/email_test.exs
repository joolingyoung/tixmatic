defmodule TixdropWeb.EmailTest do
  use ExUnit.Case
  use Bamboo.Test

  import TixdropWeb.AuthCase
  alias TixdropWeb.Email
  ...
  test "sends confirmation request email", %{email: email, key: key} do
    ...
    assert sent_email.text_body =~ "email here\n#{TixdropWeb.Endpoint.url}/confirm?key="
  ...
  test "sends reset password request email", %{email: email, key: key} do
    ...
    assert sent_email.text_body =~ "password at\n#{TixdropWeb.Endpoint.url}/password_resets/edit?key="
