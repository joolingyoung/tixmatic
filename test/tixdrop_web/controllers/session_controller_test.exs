defmodule TixdropWeb.SessionControllerTest do
  ...
  describe "login form" do
    test "rendering login form fails for user that is already logged in", %{
      ...
      assert redirected_to(conn) ==
               Routes.workspace_path(conn, :show, WorkspaceCreation.get_workspace(user))
    end
  end
  ...
  describe "create session" do
    ...
    test "login fails for user that is not yet confirmed", %{conn: conn} do
      ...
      # they are given the opportunity to resend the confirmation
      assert redirected_to(conn) ==
               Routes.confirm_path(conn, :new, Map.take(@unconfirmed_attrs, [:email]))
    end
